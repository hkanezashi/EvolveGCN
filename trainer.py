...
class Trainer:
    def __init__(self, args, splitter, gcn, classifier, comp_loss, dataset, num_classes):
        """
        :param args: Options from configparser
        :param splitter: Dataset with training, validation and testing parts
        :param gcn: GCN model instance
        :param classifier: Classifier (MLP)
        :param comp_loss: Loss function (cross entropy)
        :param dataset:
        :param num_classes:
        """
        self.args = args
        ...
        for e in range(self.args.num_epochs):
            ...
            if len(self.splitter.test) > 0 and eval_valid == best_eval_valid and e > self.args.eval_after_epochs:
                ...
                if self.args.save_node_embeddings:
                    log_file = self.logger.get_log_file_name()
    ...
    def run_epoch(self, split, epoch, set_name, grad):
        """Run training and evaluation phases in a single epoch
        :param split: Sub-dataset (train, val, or test)
        :param epoch: Epoch count
        :param set_name: Name of sub-dataset (TRAIN, VALID or TEST)
        :param grad: If True, it applies gradient
        :return: Node embeddings from the GCN model
        """
        t0 = time.time()
        log_interval = 999
        if set_name == 'TEST':
        ...
        for s in split:
            ...
            predictions, nodes_embs = self.predict(s.hist_adj_list, s.hist_ndFeats_list,
                                                   s.label_sp['idx'], s.node_mask_list)
    ...
    def predict(self, hist_adj_list, hist_ndFeats_list, node_indices, mask_list):
        """
        :param hist_adj_list: Adjacency lists
        :param hist_ndFeats_list: Node features
        :param node_indices: Node index list (edge index list for link prediction)
        :param mask_list: Node mask list
        :return: Prediction result and GNN output embeddings
        """
        nodes_embs = self.gcn(hist_adj_list, hist_ndFeats_list, mask_list)  # output: [#nodes, #feats]
        
        predict_batch_size = 100000
        gather_predictions = []
        for i in range(1 + (node_indices.size(1) // predict_batch_size)):
            cls_input = self.gather_node_embs(nodes_embs, node_indices[:, i * predict_batch_size:(i + 1) * predict_batch_size])
            predictions = self.classifier(cls_input)  # MLP: [#edges, #feats x 2] --> [#edges, 2]
            gather_predictions.append(predictions)
        gather_predictions = torch.cat(gather_predictions, dim=0)  # Concat src and dst embeddings
        return gather_predictions, nodes_embs  # [#nodes, #feats], [#edges, 2]
    
    @staticmethod
    def gather_node_embs(nodes_embs, node_indices):
        """
        :param nodes_embs: Input node embeddings [#nodes, #feats]
        :param node_indices: List(s) of nodes (edge list in link prediction tasks [2, #edges])
        :return: Concatenated node (edge) embeddings [#edges, #feats x 2]
        """
        cls_input = []
        for node_set in node_indices:
            cls_input.append(nodes_embs[node_set])
        return torch.cat(cls_input, dim=1)  # Concat src and dst embeddings
    ...
    def prepare_sample(self, sample):
        ...
        for i, adj in enumerate(sample.hist_adj_list):
            ...
            sample.node_mask_list[i] = node_mask.to(self.args.device).t()  # transposed to have same dimensions as score
